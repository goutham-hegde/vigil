...
# Published by LANL for the whole collection (csr.lanl.gov/data/cyber1). Only the
# event total is given per collection, not per file, so that is what the card checks.
PUBLISHED = {"events": 1_648_275_307, "users": 12_425, "computers": 17_684, "days": 58}
...
def render_card(p: Profile, splits: dict | None) -> str:
    ...
    match = "matches" if total == PUBLISHED["events"] else f"does NOT match ({total - PUBLISHED['events']:+,})"
    L += ["", f"Ingested rows total **{total:,}**, which {match} the {PUBLISHED['events']:,} events LANL publishes "
              f"for the collection, over {PUBLISHED['days']} days, {PUBLISHED['users']:,} users and "
              f"{PUBLISHED['computers']:,} computers."]
    rt = p.redteam
    if rt:
        L += ["", "## Red team", "",
        ...
        if "match_histogram" in rt:
            ...
            if rt["unmatched"]:
                L += ["", f"The {len(rt['unmatched'])} unmatched rows are listed in the local `profile.json`; "
                          "individual records are not reproduced here, because the dataset is not redistributed."]
