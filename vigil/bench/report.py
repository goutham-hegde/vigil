...
def _control_note(rows: list[dict], split: str) -> list[str]:
    """The NTLM caveat, carrying the measured AUC so the text cannot drift from the run.

    Every labelled red-team event in the LANL collection is NTLM/Network, so a
    bare protocol test is a near-perfect ranker by AUC and worthless at any
    alert budget. Stating its actual score turns the usual hand-wave about
    class imbalance into a number a reader can hold a model against.
    """
    row = next((e for e in rows if e["experiment"] == CONTROL and split in e["splits"]), None)
    if row is None:
        return []
    m = row["splits"][split]["metrics"]
    return ["", f"**`{CONTROL}` is a confound control, not a detector.** Every labelled red-team event is "
                f"NTLM/Network, so the one-line rule `auth_type = 'NTLM'` scores ROC-AUC "
                f"{m['roc_auc']['mean']:.3f} while flagging a large fraction of the network every day — see its "
                f"AP and alert-budget recall in the same row. Treat that AUC as the floor: a model scoring below "
                f"it has not beaten a protocol check."]
